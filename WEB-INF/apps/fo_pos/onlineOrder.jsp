...
if("calc".equals(func)) {
    ...
    try {
        ...
        String qryReady = getQuery(pgmid, "qryCalcReady"); 
        String qryReadyRun = "";
        ...
        for(int i = 0; i < arrItemlist.size(); i++) {
            ...
            row.put("comcd",ORGCD); 
            row.put("usid",USERID);
            qryReadyRun += bindVAR(qryReady,row) + "\n";
        }  
        OUTVAR.put("qryReadyRun",qryReadyRun);
        JSONObject rstReady = executeSVC(conn, qryReadyRun);
        if(!"OK".equals(getVal(rstReady,"rtnCd"))) {
            conn.rollback();
            rtnCode = getVal(rstReady,"rtnCd"); 
            rtnMsg  = getVal(rstReady,"rtnMsg");
        } else { 
...
if("savememo".equals(func)) {
    ...
    try {
        ...
        INVAR.put("usid",USERID);
        OUTVAR.put("INVAR",INVAR);
        String qry = getQuery(pgmid, "qrySavememo");   
        String qryRun = ""; 
