...
if("searchOrd".equals(func)) {
    ...
    try {  
        ...
        String qryDA = getQuery(pgmid, "qrysearchDetMVA"); 
        String qryRunDA = bindVAR(qryDA,INVAR);
        OUTVAR.put("qryRunDA",qryRunDA); //for debug
        JSONArray detA = selectSVC(conn, qryRunDA);
        OUTVAR.put("detA",detA);  
...
//saveMVO : 온라인,예약판매는 상품목록 update만
...
OUTVAR.put("qryMst",qryRun);
        ...
        if(!"OK".equals(getVal(rst,"rtnCd"))) {
            ...
            for(int i = 0; i < arrList.size(); i++) {
                ...
                row.put("lg_ord_seq",getVal(row,"lg_ord_seq"));
...
//cfmMVD : 온라인직송 일괄수락(발송) 2022.07.07
if("cfmMVD".equals(func)) {
    Connection conn = null; 
    try {  
        conn = getConn("LFN");
        conn.setAutoCommit(false);
    ...
    try {  
        ...
        */
        ...
        if(!"OK".equals(getVal(rst,"rtnCd"))) {
            ...
            OUTVAR.put("qryDetRun",qryDetRun); 
            JSONArray list = selectSVC(conn, qryDetRun);
            OUTVAR.put("list",list);
            
            /*****온라인직송 부분 수락/거부 2022.06.27***/
            String qryU = getQuery(pgmid, "udtOnlPartYN");
            String qryRunU = bindVAR(qryU,INVAR);
            rst = executeSVC(conn, qryRunU);
...
if("cancel".equals(func)) {
    ...
    try {  
        ...
        conn = getConn("LFN");
        conn.setAutoCommit(false);
        String qryDet = getQuery(pgmid, "qryCancel");
        String qryRun = "";
...
if("workYN".equals(func)) {
    ...
    try {  
        ...
        conn = getConn("LFN");
        conn.setAutoCommit(false);
        String qryDet = getQuery(pgmid, "qryWorkYN");
        String qryRun = "";
