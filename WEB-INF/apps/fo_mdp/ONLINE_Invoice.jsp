...
public String makeXML(JSONObject jo, String rootTag, int Depth) throws Exception {
    ...
    */ 
	...
	JSONObject lists = jo.DATA;
	String inv_yn = lists.send_inv_edit_yn;
    ...
    String header = "";
    header += "<HEADER>"+CR;
    header += "<SEND_COMPAYNY_ID>lfnetworks</SEND_COMPAYNY_ID>"+CR;
    header += "<SEND_AUTH_KEY><![CDATA[x96WH2C3WW19JM56yKF22N3FxVCx7W9VV3Z]]></SEND_AUTH_KEY>"+CR;
    header += "<SEND_DATE><![CDATA[" + today + "]]></SEND_DATE>"+CR;
    header += "<SEND_INV_EDIT_YN>" + inv_yn  + "</SEND_INV_DEIT_YN>";
    header += "<RESULT_TYPE></RESULT_TYPE>"+CR;
    header += "</HEADER>"+CR;
    try {
        if(Depth==0) rtn = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"+CR+header;
		...
		for(Object k : jo.keySet()) {
            ...
            if(!isNull(keys)) {
                ...
                if(v instanceof org.json.simple.JSONArray) {
                    ...
                    rtn += makeXML((JSONObject)v,keys,Depth); 
...
//OUTVAR.put("jsonObj", org.json.XML.toJSONObject(xmlStr)); 
    ...
    String xmlStr2 = makeXML(INVAR,"SABANG_INV_REGI",0); //JSONObject jo, String rootTag, Boolean bRoot)//org.json.XML.toString(INVAR);
    ...
    try {
	    ...
	    if(isNull(sUrl)) sUrl = "http://sbadmin01.sabangnet.co.kr/RTL_API/xml_order_invoice.html?xml_url=" + LFurl + fileid + ".xml";
