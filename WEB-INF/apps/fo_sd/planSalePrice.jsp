...
if("save".equals(func)) {
    ...
    try {  
        ...
        String sale_cond_no = getVal(INVAR,"sale_cond_no");
        if("TBD".equals(sale_cond_no)) {  //첫번째 저장시 수불전표번호 채번
            JSONArray arrSaleCondNo = selectSVC(conn,"SELECT SEQ_SALE_COND_NO.NEXTVAL sale_cond_no FROM DUAL");
            sale_cond_no = getVal(arrSaleCondNo,0,"sale_cond_no");
            INVAR.put("sale_cond_no",sale_cond_no);
            qryRun = bindVAR(saveMst,INVAR);  //마스터정보 MERGE(INSERT), 적용매장정보 Clear
        } else {
            qryRun = bindVAR(saveMst,INVAR);  //마스터정보 MERGE, 적용매장정보 Clear
        }
        OUTVAR.put("sale_cond_no",sale_cond_no);
        ...
        if("Y".equals(getVal(INVAR,"first_yn"))) {  //Loop의 첫 저장일때 적용매장정보 INSERT
            ...
            for(int i = 0; i < arrShop.size(); i++) {
                ...
                row.put("comcd",ORGCD);
                row.put("sale_cond_no",sale_cond_no);
                row.put("usid",USERID);            
        ...
        for(int i = 0; i < arrList.size(); i++) {
            ...
            row.put("comcd",ORGCD);
            row.put("sale_cond_no",sale_cond_no);
            row.put("sale_price_tp",getVal(INVAR,"sale_price_tp"));
            row.put("usid",USERID);
