...
if("save".equals(func)) {
    ...
    try {  
        ...
        if(!"OK".equals(getVal(rst,"rtnCd"))) {
            ...
            String dist_no = getVal(rst,"rtnMsg"); 
            ...
            for(int i = 0; i < arrList.size(); i++) {
                ...
                row.put("comcd",ORGCD); 
                row.put("usid",USERID);
                row.put("dist_no",dist_no);
                row.put("dist_dt",getVal(INVAR,"dist_dt"));
...
//merge:대상확정 이벤트처리(DB_Write)
if("merge".equals(func)) {
    Connection conn = null; 
    try {  
        conn = getConn("LFN");
        conn.setAutoCommit(false);
        ...
        String qryMst = getQuery(pgmid, "qrymerge"); 
        String qryRun = ""; 
        qryRun = bindVAR(qryMst,INVAR);  
        OUTVAR.put("qryMst",qryRun);
        JSONObject rst = executeSVC(conn, qryRun);  
