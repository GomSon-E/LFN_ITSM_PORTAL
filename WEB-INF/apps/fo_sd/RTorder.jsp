...
if("cfm".equals(func)) {
    ...
    try {  
        ...
        if(!"OK".equals(getVal(rst,"rtnCd"))) {
            ...
            String qrysaveMst = getQuery(pgmid, "qrycfm2"); 
            qry = bindVAR(qrysaveMst,INVAR);  
            rst = executeSVC(conn, qry);  
            if(!"OK".equals(getVal(rst,"rtnCd"))) {
                conn.rollback();
                rtnCode = getVal(rst,"rtnCd"); 
                rtnMsg  = getVal(rst,"rtnMsg"); 
...
//new:신규 이벤트처리(DB_Write)
if("new".equals(func)) {
    Connection conn = null; 
    try {  
        conn = getConn("LFN");
        conn.setAutoCommit(false);
        String qry = getQuery(pgmid, "qrynew");
        String qryRun = "";
    ...
    try {  
        ...
        if(!"OK".equals(getVal(rst,"rtnCd"))) {
            ...
            for(int i = 0; i < arrList.size(); i++) {
                ...
                row.put("comcd",ORGCD); 
                row.put("usid",USERID);
                //row.put("usid",getVal(INVAR,"reg_usid"));
                row.put("rotate_no",rotate_no); 
                row.put("rotate_seq",(i+1));
                row.put("rotate_dt",getVal(INVAR,"rotate_dt"));
            ...
            OUTVAR.put("rotate_no",rotate_no); 
                conn.commit();
            } 
        }
...
//requestCancel:의뢰취소 이벤트처리(DB_Write)
if("requestCancel".equals(func)) {
    Connection conn = null; 
    try {  
        conn = getConn("LFN");
        conn.setAutoCommit(false);
        //INVAR.put("comcd",ORGCD);
        //INVAR.put("usid",USERID);
        String qry = getQuery(pgmid, "qryrequestCancel");
        String qryRun = "";
        JSONArray arrList = getArray(INVAR,"det");
        for(int i = 0; i < arrList.size(); i++) {
            JSONObject row = getRow(arrList,i); 
            row.put("comcd",ORGCD); 
            row.put("usid",USERID);
            row.put("rotate_no",getVal(INVAR,"rotate_no"));
            row.put("rotate_seq",getVal(row,"rotate_seq"));
            qryRun += bindVAR(qry,row) + "\n";
        } 
        JSONObject rst = executeSVC(conn, qryRun);  
        if(!"OK".equals(getVal(rst,"rtnCd"))) {
