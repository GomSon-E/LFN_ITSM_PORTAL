...
public class EPBatchJob implements Runnable {
	...
	public void run() {
        ...
        System.out.println("deploy check v3.3");  
		...
		try {
			...
			for(int i=0; i < list.size(); i++) {
				...
				if(jobs.size()==1) {
					...
					conn.commit();
					System.out.println("job Step3 :" + getVal(job,"pgmid"));  
					
					//4.잡 호출하여 실행(url호출)
					String sUrl = "http://ep.tribons.co.kr?dir=EPBatchJob&module="+getVal(row,"module")+"&pgm="+getVal(row,"pgmid")+"&func=run";
			        ...
			        huc.connect();  
					System.out.println("job Step4 :" + getVal(job,"pgmid"));
			        
			        //5.수행결과 받기
			        int responseCode = huc.getResponseCode(); 
					System.out.println("job Step5-1 :" + getVal(job,"pgmid"));
			        //System.out.println("HourlyJob:"+ responseCode); 
			        br = new BufferedReader(new InputStreamReader(huc.getInputStream(), "UTF-8")); 
					System.out.println("job Step5-2 :" + getVal(job,"pgmid")); 
			        sb = new StringBuffer(); 
			        String temp = "";
			        ...
			        sOUTVAR = sb.toString();
					System.out.println("job Step5-3 :" + sOUTVAR); 
			        JSONParser parser = new JSONParser();
			        OUTVAR = (JSONObject) parser.parse(sOUTVAR); 
			        //System.out.println(OUTVAR.toJSONString()); 
					System.out.println("job Step5 :" + getVal(job,"pgmid"));
					...
					conn.commit();
					System.out.println("job Step7 :" + getVal(job,"pgmid"));
					...
					/* Thread Sleep사용시 톰캣 프로세스가 멈춰서 다른 request가 Hang걸려서 서버다운발생
					 * 절대 사용하지 말 것! Thread.sleep(500);
					 */
				}
			}	
