...
<% 
    /* plas.jsp 는 포털jsp로 다음의 기능을 수행한다 
...
<title>경영관리PLAS</title>
<link rel="shortcut icon" href="/images/PlasIcon.png" type="image/x-icon" />
...
<script src="/src/awecomponent.js?20220930A"  type="text/javascript" charset="utf-8"></script>

<script comment="plasportal.js로 분리" >
var gSYS = 'PLAS';
var gfn = {};
var gObj = {};
var gParam = {};
...
function gfnSendMsg(toSys, useremail, msg, url, afnCallback, ref_info, pgmid) {
	if(toSys=="naverworks") {
		...
		args.msgs = [{ email: useremail, body: msg,
		               connectInfo  : [ { title : "LFN PLAS시스템",
                                          imageUrl : nvl(url,"https://fo.lfnetworks.co.kr/plas.jsp") } ] 
					 }];
		var invar = JSON.stringify(args);		
...
function gfnMDI() { 
	...
	me.go = function(to) { 		
		...
		if(to=="home" || to==0 || isNull(to)) { 
			if(me.pageContainer.children(".framepage[pgmid='plasHome']").length > 0) {
				me.hideAll();
				me.pageContainer.children(".framepage[pgmid='plasHome']").show();
			} else {
				location.reload();
			}  
...
<% } else { %>
    ...
    <% if(!isNull(DOCID)) { %>
		//load PlasHome to .panel, 사용자정보를 조회해온다.
		const pgm = "plasHome";
		console.log("open Home with "+"<%=DOCID%>");
		gfnLoadPage(pgm,function(OUTVAR){ 
			console.log("open Home doc with "+"<%=DOCID%>");
...
<link href='https://fonts.googleapis.com/css?family=IBM Plex Sans KR' rel='stylesheet'>
<style comment="plas.css로 분리">
...
*
{
    ...
    font-family: 'IBM Plex Sans KR', sans-serif; /* 'Nanum Gothic', sans-serif; */
} 
:root
{ 
